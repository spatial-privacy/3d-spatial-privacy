...
BASE_PATH=os.path.join(BASE_DIR, 'pointnetvlad_arcore_submaps/')#'dataset/')partial_dataset
print(BASE_PATH)
...
def load_pc_file(filename):
	...
	#pc=np.load(os.path.join(BASE_PATH,filename))
    
	if(pc.shape[1]!= 4096):
		print("Error in pointcloud shape:",filename,pc.shape)
		return np.array([])
